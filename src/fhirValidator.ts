...
async function runValidatorDocker(artifactPath: string): Promise<CommandResult | null> {
    ...
    const command = `docker run --rm --name ${dockerContainer} -v ${path.resolve(
        path.dirname(artifactPath)
    )}:/data  ${imageName} ${dockerCommand}`;
...
export async function validate(fileOrJSON: string, logger = new Log("FHIR Validator")): Promise<Log> {
    ...
    if (!usingJre && !usingDocker) {
        return log.error(
            `Validator: use of option ${color.italic(
                "--validator fhirvalidator"
            )} requires Docker or JRE to execute the FHIR Validator Java application.  See: http://hl7.org/fhir/validator/`,
...
const JRE = {
    isAvailable: (): boolean => {
        ...
        if (result.exitCode === 0) {
            const version =
                /^(java \d+.+|openjdk version "\d+.+)/.exec(result.stdout || result.stderr)?.[0] ?? "unknown";
...
const Docker = {
    ...
    imageExists: async (imageName: string): Promise<boolean> => {
        return (
            (await runCommand(`docker image inspect ${imageName}`, `Check Docker image ${imageName} exists`, log))
                .exitCode === 0
        );
    },
    ...
    checkPermissions: async (): Promise<boolean> => {
        ...
        if (result.exitCode !== 0) {
            if (/permission denied/.test(result.stderr)) {
                ...
                log.error(`Docker command failed ${result.stderr}`, ErrorCode.DOCKER_ERROR);
            }
        }
        return result.exitCode === 0;
    },
    ...
    buildImage: async (dockerFile: string, imageName: string): Promise<boolean> => {
        ...
        const result = await runCommand(
            ...
            `Build Docker image ${imageName} from ${dockerFile}`,
            log
        );
