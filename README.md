...
This project provides tools to help implementers of the [SMART Health Card Framework](https://smarthealth.cards/) validate the artifacts they produce. The package's version number, currently `1.2.0-0`, matches the [specification version](https://smarthealth.cards/changelog/) the tool validates.
...
Install a specific version by specifying it as a parameter; for example, to obtain v1.2.0-0:
```
npm install github:smart-on-fhir/health-cards-dev-tools#v1.2.0-0
```
...
To validate health card artifacts, use the `shc-validator.ts` script, or simply call `node .` from the package root directory, using the desired options:
    ...
    Options:
      ...
      -V, --validator <validator>  FHIR bundle validator (choices: "default", "fhir-validator" (requires Java runtime)) 
...
This tool can now apply the HL7 FHIR Validator, in place of the default validator, with the use of the `--validator fhir-data` option. Use of the HL7 FHIR Validator requires a Java runtime. 
...
```
sudo -E env "PATH=$PATH" shc-validator --path myfhirbundle.json --type fhirbundle --validator fhir-validator
```